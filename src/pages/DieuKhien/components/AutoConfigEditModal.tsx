...
interface AutoConfigEditModalProps {
...
interface Condition {
  ...
  automationConfigId?: string | number;
}

const AutoConfigEditModal: React.FC<AutoConfigEditModalProps> = ({ setRefresh, config, deviceType }) => {
  const [loading, setLoading] = useState<boolean>(false);
  const modal = useModal();
  const [changePower, setChangePower] = useState<number>(config.changePower || 100);
  const [sensorOptions, setSensorOptions] = useState<ISensor[]>([])

  const [conditions, setConditions] = useState<Condition[]>(config.automationConfig.Condition);
  ...
  useEffect(() => {
    if (modal.isOpen && titleRef.current && descriptionRef.current) {
  ...
  const onSubmit = async () => {
    ...
    const originalIds = config.automationConfig.Condition.map(c => c.id);
    const currentIds = conditions.filter(c => c.id).map(c => c.id);
    const deletedIds = originalIds.filter(id => !currentIds.includes(id));
  
    try {
      setLoading(true);
      // 1. Update main config
      await deviceService.updateDeviceConfig(config.id, {
        name: titleRef.current?.value,
        description: descriptionRef.current?.value,
        changePower,
      });
  
      // 2. Delete removed conditions
      await Promise.all(deletedIds.map(id => deviceService.deleteDeviceAutoCondition(id)));
  
      // 3. Create new ones and update existing ones
      await Promise.all(
        payload.map(p => {
          if (!p.id) {
            return deviceService.createDeviceAutoCondition({ configId: config.id, ...p });
          } else {
            return deviceService.updateDeviceAutoCondition(p.id, p); // assuming update endpoint
          }
        })
      );
